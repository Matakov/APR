...
void Matrica::returnColumnAsVector(std::vector<double>& vec)
	{
		vec.clear();
		for(int i=0;i<this->getRow();i++) vec.push_back(this->arrayPointer[i][0]);
		return;
	}
...
void Matrica::supstitucijaUnaprijed(const Matrica& b, std::vector<double>& result)
	{
		...
		//y.printMatrix();
		for(int t=0;t<y.getRow();t++) result.push_back(y.arrayPointer[t][0]);
		//std::cout<<"Result:"<<std::endl;
		//y.returnColumnAsVector(result);
		//std::cout<<result.size();
		//for(int g=0;g<result.size();g++) std::cout<<result[g]<<" ";
		//std::cout<<std::endl;
		...
		//for(int k=0;k<num;k++) std::cout<<k<<" ";
		//std::cout<<y.getRow()<<std::endl;
		
		//std::cout<<this->getRow()<<std::endl;
		//std::cout<<result.size()<<std::endl;
		//for(int i=0;i<this->getRow();i++) std::cout<<i<<" ";//result.push_back(y.arrayPointer[i][0]);
		//for(int g=0;g<result.size();g++) std::cout<<g<<" ";//std::cout<<result[i]<<" ";
		return;
	}
	//supstitucija unazad
void Matrica::supstitucijaUnazad(const std::vector<double>& y, std::vector<double>& result)
	{
		...
		//x.printMatrix();
		result.clear();
		for(int i=0;i<y.size();i++) result.push_back(x.arrayPointer[i][0]);
		return;
	}
...
Matrica Matrica::divideScalar(int b)
	...
	//get column vector
Matrica Matrica::getColumns(int column)
	{
		Matrica A(this->row,1);
		for(int i=0;i<this->row;i++)
		{
			A.arrayPointer[i][0]=this->arrayPointer[i][column];
		}
		return A;
	}


void Matrica::getInverse(std::vector<std::vector<double>>& array)
	{
		//std::cout<<"Doing LUP"<<std::endl;
		Matrica P=this->LUPdekompozicija();
		//std::cout<<"LUP jedinicna matrica"<<std::endl;
		//P.printMatrix();
		Matrica jedinicna = this->JedinicnaMatrica();
		//std::cout<<"Jedinicna matrica"<<std::endl;
		//jedinicna.printMatrix();
		//std::cout<<"Lup matrica"<<std::endl;
		//this->printMatrix();
		for(int i=0;i<this->getRow();i++)
		{
			for(int j=0;j<this->getColumn();j++)
			{
				if((this->arrayPointer[i][j]==0 || fabs(this->arrayPointer[i][j])<1e-6) && i==j)
				{
					std::cout<<"Nema inverza!"<<std::endl;
					return;
				}
			}
		}
		int numCols = this ->getColumn();
		std::vector<std::vector<double>> inverse;
		std::vector<double> resultColumn,y;
		resultColumn.clear();
		...
		for(int i=0;i<numCols;i++)
		{
			...
			Matrica P2(P);
			Matrica temp = jedinicna.getColumns(i);
			//temp.printMatrix();
			//P2.printMatrix();
			this->supstitucijaUnaprijed(P2*temp,y);
			//std::cout<<y.size()<<std::endl;
			//for(int j=0;j<y.size();j++) std::cout<<y[j]<<" ";
			//std::cout<<std::endl;
			//std::cout<<"------------------"<<std::endl;
			this->supstitucijaUnazad(y,resultColumn);
			//std::cout<<"X vektor nakon supstitucije unazad: "<<std::endl;
			//x.printMatrix();
			inverse.push_back(resultColumn);
			resultColumn.clear();
		}
		//Matrica result(inverse);
		//result.transposeSelf();
		array.clear();
		for(int i=0;i<inverse.size();i++)
		{
			std::vector<double> tempInv(inverse[i].size(),0.0);
			array.push_back(tempInv);
		}
		for(int i=0;i<inverse.size();i++)
		{
			for(int j=0;j<inverse[0].size();j++)
			{
				array[i][j]=inverse[j][i];
				//std::cout<<inverse[j][i]<<" ";
			}
			//std::cout<<std::endl;
		}
		return;
